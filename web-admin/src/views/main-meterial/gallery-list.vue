      ...
      <el-col :span="3" v-for="(item, index) in list" :key="item.id" class="card-box-outer">
        <el-card shadow="hover" class="card-box">
          <div
            class="avatar-box"
            :title="item.url"
            :style="{ background: 'url('+ item.url +') no-repeat center', backgroundSize: 'contain' }"
          ></div>
          <div class="desc-box">
            <el-button
              size="small"
            ...
            <el-button
              ...
              type="danger"
              icon="el-icon-delete"
              @click.prevent="delImage(item, index)"
            >删除图片</el-button>
          </div>
        </el-card>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    getList() {
      http({
        url: http.adornUrl(
          `/image/query?group=${this.group}&pageNum=${this.pnumber}&pageSize=${
            this.psize
          }`
        ),
        method: "get"
        ...
        if (res.data.data.list.length > 0) {
          res.data.data.list.map(item => {
            item.url = imgUrlFormat(item.url);
            return item;
          });
